 ...
 * SMP profile
 *
 * CNF_SMP selects the execution model (config/config.h):
 *	0	single core, core 1 never started -- must remain behaviourally
 *		identical to the pre-SMP port
 *	1	dual core
 *
 * TK_MAX_CORE is the number of per-core kernel state slots.  It is 1 in the
 * single-core profile so the arrays collapse and the current-core index folds
 * to a constant zero, leaving code equivalent to the original singletons.
 *
 * Defined here rather than in profile.h because the dispatcher assembly needs
 * them and includes this header.
 */
#ifndef CNF_SMP
#define CNF_SMP			(0)
#endif
...
#if CNF_SMP
#define TK_SUPPORT_SMP		1
#define TK_MAX_CORE		2
#else
#define TK_SUPPORT_SMP		0
#define TK_MAX_CORE		1
#endif

#define ALL_CORE_BIT		((1 << TK_MAX_CORE) - 1)

/* SIO CPUID reads 0 on core 0 and 1 on core 1 (RP2040 datasheet 2.3.1.1).
   It is core-local, so reading it needs no lock or barrier. */
 ...
 * SysTick priority.
 *
 * Match the working FreeRTOS RP2040 SMP port: SysTick and PendSV share the
 * lowest priority, so the tick cannot interrupt a half-saved context. This is
 * safe now that an empty processor restores a private idle context and waits
 * in Thread mode rather than spinning inside PendSV.
 */
#define INTPRI_SYSTICK		3	/* SysTick: cannot preempt PendSV */
