...
/*
 * Processor affinity (SMP).
 *
 * For tasks, assprc names the processors on which the task may run.  Cyclic
 * and alarm handlers accept one processor bit and default to processor 1.
 * The bits follow the SMP T-Kernel convention of numbering processors from
 * 1, so TP_PRC1 is core 0.
 */
#define TA_ASSPRC	0x00010000UL	/* Honour the object's assprc field */

#define TP_PRC1		0x00000001UL	/* processor 1 (core 0) */
#define TP_PRC2		0x00000002UL	/* processor 2 (core 1) */
#define TP_ALLPRC	0x0000ffffUL	/* any processor */

IMPORT INT tk_get_prc( void );	/* processor the caller runs on, from 1 */
...
typedef struct t_ctsk {
	...
	/*
	 * Appended for SMP.  It is deliberately last: existing positional
	 * initialisers keep every earlier member at the same offset and simply
	 * leave this one zero, which reads as "no affinity specified" and is
	 * only consulted when TA_ASSPRC is set.
	 */
	UW	assprc;		/* Processor affinity mask (TA_ASSPRC) */
} T_CTSK;
...
typedef struct t_ccyc {
	...
	UB	dsname[OBJECT_NAME_LENGTH];	/* Object name */
#endif
	/* Appended so pre-SMP member offsets and positional initializers stay valid. */
	UW	assprc;		/* Assigned processor (TA_ASSPRC; one TP_PRCn bit) */
} T_CCYC;
