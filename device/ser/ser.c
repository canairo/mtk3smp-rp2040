...
#ifdef CPU_RP2040
		(void)tk_set_flg(p_buff->wait_flgid, 1);
#else
	...
	if(req->size) {
		...
		case TDN_SER_RCVTMO:		/* Receive timeout */
			*(TMO*)req->buf = p_dcb->rcv_tmo;
			break;
		case TDN_SER_COMERR:		/* Communication Error */
			DEV_SER_BUF_LOCK(p_dcb, imask);
			*(UW*)req->buf = p_dcb->com_error;
			p_dcb->com_error = 0;
			DEV_SER_BUF_UNLOCK(p_dcb, imask);
			break;
...
LOCAL ER read_data( T_SER_DCB *p_dcb, T_DEVREQ *req)
{
	...
	if(rsize != 0 ) {		/* size != 0 : Receive data */
		while(rsize) {
			DEV_SER_BUF_LOCK(p_dcb, intsts);
			if(p_buff->top != tail) {
				*pd++ = p_buff->data[tail++];
				if(tail >= DEVCONF_SER_BUFFSIZE ) tail = 0;
				p_buff->tail = tail;
				DEV_SER_BUF_UNLOCK(p_dcb, intsts);
				rsize--;
			} else {
#ifdef CPU_RP2040
				(void)tk_clr_flg(p_buff->wait_flgid, 0);
#else
				p_buff->wait_tskid = tk_get_tid();
#endif
				DEV_SER_BUF_UNLOCK(p_dcb, intsts);
#ifdef CPU_RP2040
				err = tk_wai_flg(p_buff->wait_flgid, 1,
