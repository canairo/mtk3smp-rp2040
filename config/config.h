...
/*
 * Temporary stack size, per core.
 *
 * The dispatcher runs on this stack whenever a core has no current task, with
 * interrupts enabled, so an interrupt taken at that moment runs on it too. In
 * the SMP profile that path now includes the global ready-queue assignment,
 * which is far heavier than the single-core reschedule it replaced: a 32-byte
 * exception frame plus the assignment's arrays and call chain does not fit in
 * 256 bytes with any margin. knl_tmp_stack_used() reports the true high-water
 * mark so this number can be set from measurement rather than from hope.
 */
#if CNF_SMP
