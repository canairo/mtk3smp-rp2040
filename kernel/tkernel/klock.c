  ...
  retry:
	...
	if ( !klocked ) {
		if ( loc->wtskq.next == NULL ) {
			...
			/*
			 * Reselect through the scheduler.
			 *
			 * The original assignment took the single highest
			 * priority ready task and wrote it straight into this
			 * core's slot, with no affinity check and no knowledge
			 * of what the other core is running. On two processors
			 * that can hand this core a task pinned elsewhere, or
			 * one the other core is already executing, and the
			 * damage appears later as a dispatcher restoring a
			 * context its owner is still using.
			 */
			knl_reschedule();
		}
	}
...
EXPORT void knl_UnlockOBJ( OBJLOCK *loc )
{
	...
	knl_reschedule();	/* see knl_LockOBJ */
	END_CRITICAL_SECTION;
}
