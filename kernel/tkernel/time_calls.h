...
typedef struct cyclic_handler_control_block {
	...
	UW	assprc;		/* Single legal processor bit, or 0 for default */
#if TK_SUPPORT_SMP
	UW	generation;	/* Lifetime token for detached callback reconcile */
#endif
#if USE_OBJECT_NAME
...
#if TK_SUPPORT_SMP
IMPORT void knl_distribute_cychdr( CYCCB* cyccb );
#endif
...
{
#if TK_SUPPORT_SMP
	knl_timer_insert_abs(&cyccb->cyctmeb, tm, (CBACK)knl_distribute_cychdr, cyccb);
#else
...
#if TK_SUPPORT_SMP
IMPORT void knl_distribute_almhdr( ALMCB *almcb );
#endif
...
#if TK_SUPPORT_SMP
	knl_timer_insert_abs(&almcb->almtmeb, tm, (CBACK)knl_distribute_almhdr, almcb);
#else
