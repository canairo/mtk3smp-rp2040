...
struct task_control_block {
	...
	UW	assprc;		/* Processors this task may run on (bit per core) */

	BOOL	klockwait:1;	/* TRUE at wait kernel lock */
	BOOL	klocked:1;	/* TRUE at hold kernel lock */
#if TK_SUPPORT_SMP
	BOOL	stkfree:1;	/* reclaim stack/TCB after final context save */
#endif
 ...
 *
 * knl_dispatch_disabled, knl_ctxtsk, knl_schedtsk and knl_taskindp live here
 * as per-core arrays, re-exposed as current-core lvalues under their original
 * names so existing common code is unaffected.
 */
#include "smp.h"
