...
/* Bring-up diagnostic (hw_setting.c).  Declared locally so the temporary
   diagnostic does not perturb the shared sysdepend headers. */
IMPORT void knl_diag_pulse(INT n);
#endif
...
EXPORT void Reset_Handler(void)
{
	...
	*(_UW*)SCB_SHPR3 = SCB_SHPR3_VAL;	// SysTick = 3, PendSV = 3

#if BOARD_DIAG_GPIO_SCAN
	/* CP2: vector table relocated, .data loaded, .bss cleared, low memory
	   and exception priorities configured.  Bring-up diagnostic only. */
	knl_diag_pulse(2);
#endif
